...
import net.darkmeow.asmkt.visit
import net.darkmeow.remapper.mapping.*
import net.darkmeow.remapper.util.annotations
import net.darkmeow.remapper.util.findAnnotation
import net.darkmeow.remapper.util.findAnyAnnotation
import net.darkmeow.remapper.util.findMixinAnnotation
import it.unimi.dsi.fastutil.objects.Object2ObjectOpenHashMap
import it.unimi.dsi.fastutil.objects.ObjectArrayList
import kotlinx.coroutines.coroutineScope
import kotlinx.coroutines.launch
import net.darkmeow.remapper.FastRemapperPlugin
import net.darkmeow.remapper.RemapJarTask
            ...
            launch {
                coroutineScope {
                    classEntries.forEach { classEntry ->
                        ...
                        launch {
                            targetClassList.forEach index@{
                                    val classMappingEntry = mapping[it]
                                    ?: throw IllegalStateException("Cannot find mapping for target class $it at ${classNode.name} [classMappingEntry]")
    ...
    private fun remapMixinTarget(
        ...
        val newValue = valueList.map {
            val nameTo = mapping.getNameTo(it.internalName)
                ?: throw IllegalStateException("Cannot find mapping for target class $it at ${classNode.name} [nameValue]")
        ...
        val newTargets = targetList.map {
            val nameTo = mapping.getNameTo(it)
                ?: throw IllegalStateException("Cannot find mapping for target class $it at ${classNode.name} [nameTarget]")
            if (nameTo == it) {
                it
            } else {
                remapTargetClass = true
                ...
                atTargets.forEach { ref ->
                    ...
                    fullMethodRefRegex.matchEntire(ref)?.let {
                        ...
                        val targetEntry = mapping[ownerFrom]
                            ?: throw IllegalStateException("Cannot find mapping[targetClass=$targetClass, ownerFrom=$ownerFrom, nameFrom=$nameFrom, descFrom=$descFrom, at=method]")
                        ...
                        val targetEntry = mapping[ownerFrom]
                            ?: throw IllegalStateException("Cannot find mapping[targetClass=$targetClass, ownerFrom=$ownerFrom, nameFrom=$nameFrom, descFrom=$descFrom, at=field]")
                        val ownerTo = targetEntry.nameTo
                    ...
                    FastRemapperPlugin.logger.info("ClassRefMap[ref=$ref, value=${classRefmap[ref]}]")
                }
            }
